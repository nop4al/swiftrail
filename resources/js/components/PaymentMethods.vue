...
// Midtrans / QR handling (QR modal kept for future use)
...
const fetchPaymentMethods = async () => {
    try {
        ...
        errorMessage.value =
            error.message ||
            "Gagal memuat metode pembayaran. Silakan coba lagi.";
...
const doPay = () => {
    if (selected.value === "swiftpay" && !swiftSufficient()) {
        showError(
            new Error("Saldo SwiftPay tidak mencukupi"),
            "Metode Pembayaran"
        );
        return;
    }
    ...
    <div class="payment-page">
        ...
        <main class="content-wrapper">
            ...
            <div v-if="isLoading" class="loading-state">
                ...
                <p>
                    Tunggu sebentar, kami sedang mempersiapkan pilihan
                    pembayaran Anda
                </p>
            </div>
            ...
            <div v-else-if="hasError" class="error-state">
                ...
                <p>{{ errorMessage }}</p>
                <button class="retry-btn" @click="fetchPaymentMethods">
                    Coba Lagi
                </button>
            </div>
            ...
            <div v-else class="payment-content">
                <div class="card">
                    ...
                    <div
                        class="method swiftpay"
                        :class="{ active: selected === 'swiftpay' }"
                        @click="selected = 'swiftpay'"
                    >
                        <div class="method-left">
                            <div class="method-title">SwiftPay</div>
                            <div class="method-sub">
                                Saldo SwiftPay: Rp
                                {{ swiftBalance.toLocaleString("id-ID") }}
                            </div>
                        </div>
                        <div class="method-right">
                            <div class="balance-badge">
                                Rp {{ swiftBalance.toLocaleString("id-ID") }}
                            </div>
                    ...
                    <div
                        class="method"
                        :class="{ active: selected === 'midtrans' }"
                        @click="selected = 'midtrans'"
                    >
                        <div class="method-left">
                            <div class="method-title">Midtrans</div>
                            <div class="method-sub">
                                Bayar via kartu, e-wallet, atau transfer
                                (Midtrans)
                            </div>
                        </div>
                        <div class="method-right">
                            {{ selected === "midtrans" ? "Terpilih" : "Pilih" }}
                        </div>
                    </div>
                </div>
                ...
                <div class="summary-card">
                    ...
                    <div class="summary-actions">
                        <button
                            class="pay-action"
                            :disabled="
                                selected === 'swiftpay' && !swiftSufficient()
                            "
                            @click="doPay"
                        >
                            PAY
                        ...
                        <div
                            v-if="selected === 'swiftpay' && !swiftSufficient()"
                            class="warning"
                        >
                            Saldo SwiftPay tidak mencukupi untuk pembayaran.
                        </div>
                    </div>
                </div>
            </div>
...
.method {
    ...
    border-radius: 8px;
    cursor: pointer;
    border: 1px solid rgba(14, 23, 36, 0.04);
    margin-bottom: 8px;
}
.method.active {
    box-shadow: 0 4px 20px rgba(22, 117, 231, 0.12);
    border-color: rgba(22, 117, 231, 0.12);
}
.method-title {
    font-weight: 700;
}
...
.method.swiftpay {
    background: linear-gradient(135deg, #5dd6ff, #3a8dff);
    color: #fff;
    border: none;
}
.method.swiftpay .method-sub {
...
.summary-row.label-left .amount {
    font-size: 16px;
    font-weight: 800;
}
.summary-row.label-left .label {
    color: var(--color-text-light);
    font-size: 14px;
    font-weight: 600;
}
