...
const qrisLogo =
    'data:image/svg+xml,%3Csvg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"%3E%3Crect width="100" height="100" fill="%23000080"/%3E%3Ctext x="50" y="50" text-anchor="middle" dominant-baseline="middle" fill="white" font-size="8" font-weight="bold"%3EQRIS%3C/text%3E%3C/svg%3E';
...
const generateQRCode = async () => {
    try {
        const qrData =
            `00020126360014com.midtrans${orderNumber}${total}`.substring(
                0,
                120
            );
...
const processPaymentTransaction = async () => {
    try {
        ...
        if (!authToken) {
            throw new Error(
                "Token authentikasi tidak ditemukan. Silakan login kembali."
            );
        }
        ...
        try {
            ...
            const bookingPayload = {
                ...
                user_id: userId,
                passenger_name:
                    userProfile.name || passengerName || "Penumpang",
            ...
            try {
                ...
                if (!bookingRes.ok) {
                    ...
                    console.error(
                        `Failed to create booking via API (${bookingRes.status}):`,
                        errorData
                    );
                    ...
                    if (!bookingResponse) {
                        throw new Error(
                            "Gagal membuat pemesanan. Semua kursi sedang penuh. Silakan coba lagi."
                        );
                    }
                } else {
            ...
            // Integrate Midtrans: request token/redirect from backend then open Snap or redirect
            try {
                const token =
                    localStorage.getItem("auth_token") ||
                    sessionStorage.getItem("auth_token");
                ...
                const midPayload = {
                    ...
                    user_id: userId,
                    booking_id:
                        bookingResponse?.data?.id || bookingPayload.booking_id,
                    customer: {
                        name: userProfile.name || passengerName || "Penumpang",
                        email: userProfile.email || "customer@swiftrail.com",
                    },
                };

                console.log(
                    "Calling Midtrans create with payload:",
                    JSON.stringify(midPayload, null, 2)
                );

                // If payment method is SwiftPay, call wallet payment endpoint instead
                if (paymentMethod === "swiftpay") {
                    try {
                        const swiftRes = await fetch(
                            "/api/v1/swiftpay/payment",
                            {
                                method: "POST",
                                headers: {
                                    "Content-Type": "application/json",
                        ...
                        if (!swiftRes.ok) {
                            throw new Error(
                                swiftData.message || "Pembayaran SwiftPay gagal"
                            );
                        }
                        ...
                        showSuccess(
                            "Pembayaran berhasil diproses melalui SwiftPay"
                        );
                        ...
                        if (bookingResponse?.data) {
                            ...
                            try {
                                ...
                                await fetch(
                                    `/api/v1/bookings/${bookingCode}/confirm-payment`,
                                    {
                                        method: "POST",
                                        headers: {
                                            "Content-Type": "application/json",
                                ...
                                console.error(
                                    "Error confirming booking after swiftpay:",
                                    confirmError
                                );
                            }
                        }
                        ...
                        console.error("SwiftPay payment error:", swErr);
                        hasError.value = true;
                        errorMessage.value =
                            swErr.message ||
                            "Gagal memproses pembayaran via SwiftPay";
                        isProcessing.value = false;
                        showError(swErr, "SwiftPay");
                        return;
                    }
                }

                const midRes = await fetch("/api/v1/midtrans/create", {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                        Authorization: token ? `Bearer ${token}` : "",
                    },
                    body: JSON.stringify(midPayload),
                });

                const midData = await midRes.json();
                console.log("Midtrans response:", midData);

                if (!midRes.ok) {
                    throw new Error(
                        midData.message || "Gagal membuat transaksi Midtrans"
                    );
                }

                const token_snap = midData.token ?? midData.data?.token ?? null;
                const redirectUrl =
                    midData.redirect_url ?? midData.data?.redirect_url ?? null;

                // If Midtrans returns a snap token, load snap.js and open the payment popup
                if (token_snap) {
                    // Load Snap JS if necessary
                    if (!window.snap) {
                        await new Promise((resolve, reject) => {
                            const script = document.createElement("script");
                            script.src =
                                "https://app.sandbox.midtrans.com/snap/snap.js";
                            // Public client key (sandbox). Replace if you use production.
                            script.setAttribute(
                                "data-client-key",
                                "SB-Mid-client-3nS_QmQqFF714vxt"
                            );
                            script.async = true;
                            script.onload = () => resolve();
                            script.onerror = () =>
                                reject(
                                    new Error("Failed to load Midtrans Snap")
                                );
                            document.head.appendChild(script);
                        });
                    }

                    // Small delay to ensure Snap initialized
                    ...
                    if (window.snap && typeof window.snap.pay === "function") {
                        window.snap.pay(token_snap, {
                            onSuccess: async function (result) {
                                ...
                                if (bookingResponse?.data) {
                                    ...
                                    try {
                                        ...
                                        console.error(
                                            "Error confirming booking after midtrans:",
                                            confirmError
                                        );
                                    }
                                }
                            ...
                            onPending: function (result) {
                                ...
                                // You can redirect to a pending/awaiting page or show message
                            },
                        ...
                        // Fallback: if snap not available but redirect URL provided
                        window.location.href = redirectUrl;
                    } else {
                        throw new Error(
                            "Midtrans Snap tidak tersedia dan tidak ada redirect URL"
                        );
                    }
                } else if (redirectUrl) {
                    // Redirect flow (e.g., bank redirect)
                    window.location.href = redirectUrl;
                } else {
                    throw new Error(
                        "Respons Midtrans tidak berisi token atau redirect URL"
                    );
                }
            } catch (midErr) {
                ...
                hasError.value = true;
                errorMessage.value =
                    midErr.message || "Gagal memproses pembayaran via Midtrans";
                isProcessing.value = false;
                showError(midErr, "Midtrans");
            }
        } catch (paymentError) {
        ...
        hasError.value = true;
        errorMessage.value =
            error.message || "Gagal memproses pembayaran. Silakan coba lagi.";
...
<template>
    <div class="checkout-page">
        ...
        <header class="header">
            <button class="back-btn" @click="goBack" aria-label="Kembali">
                ...
                >
                    <path
                        d="M10 17L5 12M5 12L10 7M5 12H20"
                        stroke="currentColor"
                        stroke-width="2"
        ...
        <main class="content-wrapper">
            ...
            <div class="payment-method-card">
                ...
                <div class="method-display">
                    <div class="method-icon">
                        ...
                        />
                        <svg
                            v-else-if="paymentMethod === 'swiftpay'"
                            width="24"
                            height="24"
                            viewBox="0 0 24 24"
                            fill="currentColor"
            ...
            <div v-if="paymentMethod === 'ib'" class="payment-code-card">
                ...
                <div class="code-display">
                    <div class="code-info">
                        <p class="code-label">
                            Masukkan kode ini di {{ selectedBank }}:
                        </p>
                        <div class="code-box">
                    ...
                    <p class="code-instruction">
                        Gunakan kode ini untuk transfer via Internet Banking
                        {{ selectedBank }}. Pastikan nominal sesuai: Rp
            ...
            <div class="passenger-info-card" v-if="passengerName">
                ...
                <div class="passenger-details">
                    ...
                    <div class="passenger-breakdown">
                        <div v-if="adultCount > 0" class="breakdown-item">
                            {{ adultCount }} Dewasa
                        </div>
            ...
            <div class="detail-harga-card">
                ...
                <div class="detail-item">
                    <div class="detail-label">
                        <div class="train-info-block">
                            <div class="train-header">
                                ...
                                <span class="train-number" v-if="trainNumber"
                                    >#{{ trainNumber }}</span
                                >
                            </div>
                            <div class="train-route-info">
                                <span class="route"
                                    >{{ fromStation }} → {{ toStation }}</span
                                >
                            </div>
                    ...
                    <div class="detail-value">
                        ...
                        <span class="price"
                            >Rp {{ ticketPrice.toLocaleString("id-ID") }}</span
                        >
                    </div>
                </div>
    ...
    <div
        v-if="showQRModal && paymentMethod === 'qris'"
            ...
            <div class="qr-modal-body">
                ...
                <div class="qr-info">
                    ...
                    <p class="qr-instruction">
                        Tunjukkan kode QR ini ke kasir atau pindai dengan
...
.total-payment {
    ...
    background: linear-gradient(
        135deg,
        rgba(22, 117, 231, 0.08),
        rgba(52, 156, 255, 0.08)
    );
    border-radius: 10px;
    margin-top: 12px;
}
